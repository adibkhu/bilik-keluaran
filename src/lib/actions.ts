...
function catchAuthError(err: unknown): ActionResult {
  return {
...
export async function createPost(formData: FormData): Promise<ActionResult> {
  try {
    ...
    if (!(postType in POST_TYPE_LABELS)) {
    ...
    if (trackUrl && !isSupportedTrackUrl(trackUrl)) {
      return {
        error:
          "Track URL must be Spotify, YouTube, SoundCloud, or Apple Music.",
      };
    }
    ...
    if (trackUrl) {
      try {
        ...
        return {
          error: "Could not fetch track preview. Check the URL and try again.",
        };
      }
    }
